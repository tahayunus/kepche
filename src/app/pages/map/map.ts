...
export class MapPage extends BasePage {
  ...
  async ngOnInit() {
    ...
    const secondcategoryId = this.getQueryParams().secondcategory;
    ...
    if (secondcategoryId) {
      ...
      this.params.secondcategory = secondcategory;
    }
  }
  ...
  async loadSearchData(query:any) {

    try {

      this.showLoadingView({ showOverlay: false });
      this.params.search = query;
      this.places = await this.placeService.loadSeach(this.params);
  ...
  onPlaceTouched(place: Place) {
    this.navigateToRelative('../map/' + place.id + '/' + place.slug);
  }
